...
traincont<-as.data.frame(cont[[2]][1:10000,])
traincat<-as.data.frame(cat1[1:10000,])
testcont<-as.data.frame(cont[[2]][20001:30000,])
testcat<-as.data.frame(cat1[20001:30000,])
d2<-diag(1000)
for(i in 1:999)
{
  for(l in (i+1):1000)
  {
    d2[i,l]<-sum((traincont[i,]-traincont[l,])^2*contweight)+sum((traincat[i,]-traincat[l,])^2*catweight)
  }
  print(i)
}
for(i in 1:1000)
...
require(lle)
calc_k( sub2[1:1000,1:254], 2, 1, 15, TRUE, TRUE, 4 )
results <- lle( X=sub2[1:5000,-255], m=2, k=14, reg=1, ss=FALSE,  v=0.9 ) 
d2<-results$Y
require(ggplot2)
qplot(d2[,1],d2[,2],col=sub2[1:5000,255],xlim = c(-2.5,2.5),ylim = c(-2.5,2.5),xlab = "x",ylab = "y",main = "locally linear embedding")
pca<-prcomp(sub2[1:5000,-255])
d22<-pca$x[,1:2]
qplot(d22[,1],d22[,2],col=sub2[1:5000,255],xlab = "x",ylab = "y",main = "principal component analysis")
